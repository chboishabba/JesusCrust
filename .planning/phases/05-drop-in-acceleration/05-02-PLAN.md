...
<task type="auto">
  ...
  <files>prototypes/dropin/example.js</files>
  <action>Write a script that uses the adapter to simulate an existing app issuing DOM writes before each tick. Show two ticks with different text, log serialized DOM/fingerprint, and ensure each tick produces exactly one commit. Document expected output in comments.</action>
  <verify>`node prototypes/dropin/example.js` runs, prints serialization/fingerprint twice, and does not throw.</verify>
  <done>Prototype demonstrates wrap of DOM writes and deterministic batches across ticks.</done>
</task>
