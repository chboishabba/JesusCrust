...
function recordCommit({
  ...
  const entry = {
    ...
    durations: {
      ...
      style_ms: durations.style_ms ?? durations.style_recalc_ms ?? 0,
      style_recalc_ms: durations.style_recalc_ms ?? durations.style_ms ?? 0,
      selector_invalidation_ms: durations.selector_invalidation_ms ?? 0,
      layout_ms: durations.layout_ms ?? 0,
      render_ms: durations.render_ms ?? 0,
      total_ms: durations.total_ms ?? duration,
    },
    work: {
      dom_mutations: work.dom_mutations ?? work.patch_ops ?? 0,
      nodes_touched: work.nodes_touched ?? 0,
      selectors_evaluated: work.selectors_evaluated ?? 0,
      elements_invalidated: work.elements_invalidated ?? work.invalidated_elements ?? 0,
      restyled_elements: work.restyled_elements ?? work.elements_restyled ?? 0,
      patch_bytes: work.patch_bytes ?? patchSize,
      patch_ops: work.patch_ops ?? work.dom_mutations ?? 0,
    },
    guardrail,
    timestamp: Date.now(),
  };
