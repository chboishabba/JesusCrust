...
- Phase 5 execution (2026-01-14): completed Preact renderer integration (`prototypes/dropin/preact-renderer.js`, `preact-example.js`) and documented renderer expectations in discovery + the adapter contract.
- Phase 6 planning (2026-01-14): `.planning/phases/06-browser-rollout/06-01-PLAN.md` now includes the architecture/responsibility matrix tying Verso, Servo, and SpiderMonkey together, guardrail tasks that forbid weakening Phase-5 semantics, and notes about telemetry placement; `docs/phase6_pr_boundaries.md` spells out which changes belong in policy/UI (Verso) versus semantics/parallelism (Servo/adapter), mandates `npm run conformance`, and requires “semantic impact” notes for adapter/contract/renderer shims changes.
- Phase 6 rollout: shipped the guarded host UI + telemetry surfaces (`prototypes/browser-extension/{host,telemetry,ui.{html,js}}`), documented the API/manifest, added `manifest.json`, and recorded an e2e fallback/reset test script that runs via `node prototypes/browser-extension/test-host-flow.js`.
- Phase 6 instrumentation planning: created `.planning/phases/06-browser-rollout/06-02-PLAN.md` to gate Servo selector telemetry behind a feature flag, extend the Phase-6 diagnostics UI with selector/guardrail/fingerprint graphs, and stage the selector invalidation PR once telemetry proves selector work dominates ticks.
- Phase 6 framing update (2026-01-17): reframed the thesis as off-main-thread compute with a deterministic commit budget; updated README, PROJECT_CONTEXT, `docs/phase6_README.md`, and planning docs to reflect the multithread-first headline.
- Phase 6 planning (2026-01-17): added `.planning/phases/06-browser-rollout/06-04-PLAN.md` for the worker protocol/commit budget and documented telemetry sample storage in `.planning/phases/06-browser-rollout/telemetry-samples/README.md`.
